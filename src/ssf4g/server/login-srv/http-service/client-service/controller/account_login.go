...
func AccountLogin(w http.ResponseWriter, r *http.Request) {
	...
	if strAccntName != "" {
		...
		tlog.Error("account login controller (%v) err (accnt_name nil).", r)
	...
	if strAccntPass != "" {
		...
		tlog.Error("account login controller (%v) err (accnt_pass nil).", r)
	...
	errData := clientmodel.AccountLogin(w, accntName, accntPass, realIP)

	if errData != nil {
		tlog.Error("account login controller (%v) err (model %v).", r, errData.Error())
